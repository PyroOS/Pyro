...
#define ACPI_BUS_VERSION 1

typedef struct 
{
	void		( *poweroff )( void );
	acpi_status ( *evaluate_object )( acpi_handle object, acpi_string pathname, struct acpi_object_list *parameter_objects, struct acpi_buffer *return_object_buffer );
	acpi_status ( *evaluate_integer )( acpi_handle handle, acpi_string pathname, struct acpi_object_list *arguments, unsigned long *data );
	acpi_status ( *extract_package )( union acpi_object *package, struct acpi_buffer *format, struct acpi_buffer *buffer );
	acpi_status ( *get_name )( acpi_handle handle, u32 name_type, struct acpi_buffer *ret_path_ptr );
