...
void emu10k1_set_volume_gpr(struct emu10k1_card *card, int addr, int32 vol, int scale)
{
	struct patch_manager *mgr = &card->mgr;
	unsigned long flags;
	int muting;

	static const int32 log2lin[4] ={           //  attenuation (dB)
		0x7fffffff,                      //       0.0         
	...
	spinlock_cli(&mgr->lock, flags);
	emu10k1_set_control_gpr(card, addr, vol, 0);
	spinunlock_restore(&mgr->lock, flags);
}
