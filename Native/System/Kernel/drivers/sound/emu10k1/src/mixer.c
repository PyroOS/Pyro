...
static const uint32 bass_table[41][5] = {
...
static const uint32 treble_table[41][5] = {
...
/*static int emu10k1_private_mixer(struct emu10k1_card *card, unsigned int cmd, unsigned long arg)
{
	...
	case SOUND_MIXER_PRIVATE3:
		...
		ctl = (struct mixer_private_ioctl *) kmalloc(sizeof(struct mixer_private_ioctl), MEMF_KERNEL);
		if (ctl == NULL)
			return -ENOMEM;

		if (memcpy_from_user(ctl, (void *) arg, sizeof(struct mixer_private_ioctl))) {
			kfree(ctl);
			return -EFAULT;
		}
		...
		case CMD_GETRECSRC:
			ctl->val[0] = card->wavein.recsrc;
			ctl->val[1] = card->wavein.fxwc;
			if (memcpy_to_user((void *) arg, ctl, sizeof(struct mixer_private_ioctl)))
				ret = -EFAULT;
		...
		case CMD_GETPATCH:
			if (ctl->val[0] == 0) {
				if (memcpy_to_user((void *) arg, &card->mgr.rpatch, sizeof(struct dsp_rpatch)))
                                	ret = -EFAULT;
			} else {
				if ((ctl->val[0] - 1) / PATCHES_PER_PAGE >= card->mgr.current_pages) {
					ret = -EINVAL;
				...
				if (memcpy_to_user((void *) arg, PATCH(&card->mgr, ctl->val[0] - 1), sizeof(struct dsp_patch)))
					ret = -EFAULT;
			}
		...
		case CMD_GETGPR:
			...
			if (memcpy_to_user((void *) arg, &card->mgr.gpr[id], sizeof(struct dsp_gpr)))
				ret = -EFAULT;
		...
		case CMD_SETPATCH:
			if (ctl->val[0] == 0)
				...
				if (page >= card->mgr.current_pages) {
					for (i = card->mgr.current_pages; i < page + 1; i++) {
								card->mgr.patch[i] = kmalloc(PAGE_SIZE,MEMF_KERNEL|MEMF_CLEAR);
						if(card->mgr.patch[i] == NULL) {
							card->mgr.current_pages = i;
							ret = -ENOMEM;
							break;
		...
		case CMD_SETCTLGPR:
			addr = emu10k1_find_control_gpr(&card->mgr, (char *) ctl->val, (char *) ctl->val + PATCH_NAME_SIZE);
			emu10k1_set_control_gpr(card, addr, *((int32 *)((char *) ctl->val + 2 * PATCH_NAME_SIZE)), 0);
			break;
	...
	case SOUND_MIXER_PRIVATE4:
		...
		if (memcpy_from_user(&size, (void *) arg, sizeof(size)))
			return -EFAULT;
		...
		if (size != card->tankmem.size) {
			...
			if (size != 0) {
				card->tankmem.addr = alloc_dma_mem(card->pci_dev, size, &card->tankmem.dma_handle);
				if (card->tankmem.addr == NULL)
					return -ENOMEM;

				card->tankmem.size = size;

				sblive_writeptr_tag(card, 0, TCB,(uint32) card->tankmem.dma_handle, TCBS, size_reg, TAGLIST_END);
...
status_t emu10k1_mixer_open(void *node, uint32 flags, void **cookie)
{
	return 0;
}

status_t emu10k1_mixer_release(void *node, void *cookie)
{
	DPF(4, "emu10k1_mixer_release()\n");
	return 0;
}

DeviceOperations_s emu10k1_mixer_fops = {
		emu10k1_mixer_open,
