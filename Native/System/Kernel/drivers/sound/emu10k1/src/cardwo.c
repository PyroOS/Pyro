...
static int get_voice(struct emu10k1_card *card, struct woinst *woinst, unsigned int voicenum)
{
	...
	/* Calculate pitch */
	voice->initial_pitch = (uint16) (srToPitch(woinst->format.samplingrate) >> 8);
 ...
 *
 */
void emu10k1_waveout_getxfersize(struct woinst *woinst, uint32 *total_free_bytes)
{
	struct waveout_buffer *buffer = &woinst->buffer;
	int pending_bytes;
 ...
 *
 */
static void copy_block(void **dst, uint32 str, uint8 *src, uint32 len)
{
	unsigned int pg;
	unsigned int pgoff;
	unsigned int k;
	...
	if (len > PAGE_SIZE - pgoff) {
		k = PAGE_SIZE - pgoff;
		memcpy_from_user((uint8 *)dst[pg] + pgoff, src, k);
		len -= k;
		while (len > PAGE_SIZE) {
			memcpy_from_user(dst[++pg], src + k, PAGE_SIZE);
			k += PAGE_SIZE;
			len -= PAGE_SIZE;
		}
		memcpy_from_user(dst[++pg], src + k, len);

	} else
		memcpy_from_user((uint8 *)dst[pg] + pgoff, src, len);
}
 ...
 *
 */
static void copy_ilv_block(struct woinst *woinst, uint32 str, uint8 *src, uint32 len) 
{
	unsigned int pg;
	unsigned int pgoff;
	...
	while (len) { 
		for (voice_num = 0; voice_num < woinst->num_voices; voice_num++) {
			memcpy_from_user((uint8 *)(voice[voice_num].mem.addr[pg]) + pgoff, src, woinst->format.bytespervoicesample);
 ...
 *
 */
static void fill_block(struct woinst *woinst, uint32 str, uint8 data, uint32 len)
{
	unsigned int pg;
	unsigned int pgoff;
	unsigned int voice_num;
	...
	if (len > PAGE_SIZE - pgoff) {
		k = PAGE_SIZE - pgoff;
		for (voice_num = 0; voice_num < woinst->num_voices; voice_num++)
			memset((uint8 *)voice[voice_num].mem.addr[pg] + pgoff, data, k);
		len -= k;
		while (len > PAGE_SIZE) {
			pg++;
			for (voice_num = 0; voice_num < woinst->num_voices; voice_num++)
		...
		for (voice_num = 0; voice_num < woinst->num_voices; voice_num++)
			memset((uint8 *)voice[voice_num].mem.addr[pg] + pgoff, data, len);
	}
}
 ...
 *
 */
void emu10k1_waveout_xferdata(struct woinst *woinst, uint8 *data, uint32 *size)
{
	struct waveout_buffer *buffer = &woinst->buffer;
	struct voice_mem *mem = &woinst->voice[0].mem;
	uint32 sizetocopy, sizetocopy_now, start;
	unsigned long flags;

	sizetocopy = min_t(uint32, buffer->size, *size);
	*size = sizetocopy;
