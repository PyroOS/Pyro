/*
 *  The Syllable kernel
 *	VMware graphics kernel driver
 *	Copyright (C) 2004 James Hayhurst
...
#define APPSERVER_DRIVER "vmware"

struct gfx_device g_sDevices[] = {
	{PCI_VENDOR_ID_VMWARE, PCI_DEVICE_ID_VMWARE_SVGA, "VMware", "VMware_SVGA"},
	{PCI_VENDOR_ID_VMWARE, PCI_DEVICE_ID_VMWARE_SVGA2, "VMware", "VMware_SVGA2"},
};
	...
	for ( nPCINum = 0; psBus->get_pci_info( &sInfo, nPCINum ) == 0; ++nPCINum )
	{
		for ( nDeviceNum = 0; nDeviceNum < nNumDevices; ++nDeviceNum )
		{
			...
			if ( sInfo.nVendorID == g_sDevices[nDeviceNum].nVendorID && sInfo.nDeviceID == g_sDevices[nDeviceNum].nDeviceID )
			{
				...
				/* Create node path */
				sprintf( zNodePath, "graphics/vmware_%i_0x%x_0x%x", nPCINum, ( uint )sInfo.nVendorID, ( uint )sInfo.nDeviceID );
