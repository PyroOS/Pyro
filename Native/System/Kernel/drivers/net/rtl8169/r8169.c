...
struct rtl8169_private {
	...
	SpinLock_s lock;		/* spin lock flag */
	uint32 msg_enable;
	int chipset;
	int mac_version;
	int phy_version;
...
static void rtl8169_check_link_status(struct net_device *dev, struct rtl8169_private *tp, void *ioaddr)
{
	...
	if (tp->link_ok(ioaddr)) {
		...
		kerndbg(KERN_INFO, "%s: link up\n", dev->name);
	} else {
		kerndbg(KERN_INFO, "%s: link down\n", dev->name);
...
static int rtl8169_set_speed_tbi(struct net_device *dev, uint8 autoneg, uint16 speed, uint8 duplex)
{
	...
	if ((autoneg == AUTONEG_DISABLE) && (speed == SPEED_1000) &&
		...
		kerndbg( KERN_WARNING, "%s: incorrect speed setting refused in TBI mode\n", dev->name);
		ret = -EOPNOTSUPP;
	}
...
static int rtl8169_set_speed(struct net_device *dev, uint8 autoneg, uint16 speed, uint8 duplex)
{
	...
	if (netif_running(dev) && (tp->phy_1000_ctrl_reg & PHY_Cap_1000_Full))
	{
		...
		start_timer(tp->timer, (timer_callback *) &rtl8169_phy_timer, dev, (jiffies + RTL8169_PHY_TIMEOUT)*100, true );
	}
...
static void rtl8169_print_phy_version(struct rtl8169_private *tp)
{
	...
	for (p = phy_print; p->msg; p++) {
		if (tp->phy_version == p->version) {
			kerndbg( KERN_DEBUG, "phy_version == %s (%04x)\n", p->msg, p->reg);
			return;
		}
	}
	kerndbg( KERN_DEBUG, "phy_version == Unknown\n");
}
...
static void rtl8169_hw_phy_config(struct net_device *dev)
{
	...
	kerndbg( KERN_DEBUG, "MAC version != 0 && PHY version == 0 or 1\n");
	kerndbg( KERN_DEBUG, "Do final_reg2.cfg\n");
...
static void rtl8169_phy_timer(unsigned long __opaque)
{
	...
	kerndbg(KERN_WARNING, "%s: PHY reset until link up\n", dev->name);
...
out_mod_timer:
	...
	tp->timer = create_timer();
	start_timer(tp->timer, (timer_callback *) &rtl8169_phy_timer, dev, (jiffies + timeout)*100, true );
out_unlock:
	spinunlock(&tp->lock);
}
...
static int rtl8169_init_board(PCI_Info_s *pdev, struct net_device *dev, void **ioaddr_out)
{
	...
	if( tp->reg_area < 0 ) {
		kerndbg ( KERN_DEBUG, "r8169: failed to create register area (%d)\n", tp->reg_area);
		goto out;
	}

	if( remap_area (tp->reg_area, (void *)(addr & PAGE_MASK)) < 0 ) {
		kerndbg( KERN_DEBUG, "r8169: failed to remap register area (%d)\n", tp->reg_area);
		goto out;
	}
	...
	if (i < 0) {
		...
		kerndbg( KERN_DEBUG, "r8169: unknown chip version, assuming %s\n", rtl_chip_info[0].name);
		i++;
	}
	tp->chipset = i;
...
static int rtl8169_init_one(PCI_Info_s *pdev, struct net_device *dev)
{
	...
	kerndbg(KERN_DEBUG, "%s: Identified chip type is '%s'.\n", dev->name, rtl_chip_info[tp->chipset].name);

	kerndbg(KERN_DEBUG, "%s: RTL8169 at 0x%lx, "
	...
	if (RTL_R8(PHYstatus) & TBI_Enable)
		kerndbg( KERN_DEBUG, "%s: TBI auto-negotiating\n", dev->name);
...
static int rtl8169_open(struct net_device *dev)
{
	...
	tp->RxDesc = kmalloc( R8169_RX_RING_BYTES, MEMF_KERNEL );
	tp->RxDescArray = ALIGN( tp->RxDesc, 255 );
...
static void rtl8169_pcierr_interrupt(struct net_device *dev)
{
	...
	kerndbg( KERN_DEBUG_LOW, "%s: PCI error (cmd = 0x%04x, status = 0x%04x).\n", dev->name, pci_cmd, pci_status);
	...
	if ((tp->cp_cmd & PCIDAC) && !tp->dirty_rx && !tp->cur_rx) {
		kerndbg( KERN_DEBUG, "%s: disabling PCI DAC.\n", dev->name);
		tp->cp_cmd &= ~PCIDAC;
		RTL_W16(CPlusCmd, tp->cp_cmd);
		rtl8169_schedule_work(dev, rtl8169_reinit_task);
	}
...
static void rtl8169_tx_interrupt(struct net_device *dev, struct rtl8169_private *tp, void *ioaddr)
{
	...
	kerndbg( KERN_DEBUG_LOW, "rtl8169_tx_interrupt()\n" );
...
static int rtl8169_rx_interrupt(struct net_device *dev, struct rtl8169_private *tp, void *ioaddr)
{
	...
	for (; rx_left > 0; rx_left--, cur_rx++) {
		...
		if (status & DescOwn)
			break;
		if (status & RxRES) {
			kerndbg( KERN_DEBUG, "%s: Rx ERROR. status = %08x\n", dev->name, status);
	...
	if (!delta && count )
		kerndbg( KERN_DEBUG, "%s: no Rx buffer allocated\n", dev->name);
	tp->dirty_rx += delta;
	...
	if (tp->dirty_rx + NUM_RX_DESC == tp->cur_rx)
		kerndbg( KERN_WARNING, "%s: Rx buffers exhausted\n", dev->name);
...
static int rtl8169_interrupt(int irq, void *dev_instance, SysCallRegs_s *regs)
{
	...
	do {
		...
		kerndbg( KERN_DEBUG_LOW, "rtl8169_interrupt() status is 0x%.8x\n", status );
	...
	if (boguscnt <= 0) {
		kerndbg( KERN_WARNING, "%s: Too much work at interrupt!\n", dev->name);
...
static status_t r8169_ioctl( void* pNode, void* pCookie, uint32 nCommand, void* pArgs, bool bFromKernel )
{
    ...
    switch( nCommand )
    {
		...
		case SIOCGIFHWADDR:
			...
			break;
		default:
			kerndbg( KERN_WARNING, "r8169_ioctl() unknown command %d\n", (int)nCommand );
			nError = -ENOSYS;
			break;
    }
    ...
    for ( i = 0 ; g_psBus->get_pci_info( &sInfo, i ) == 0 ; ++i )
	{
		...
		for ( chip_idx = 0 ; rtl8169_pci_tbl[chip_idx].vendor_id ; chip_idx++ )
		{
				 ...
				 sInfo.nDeviceID == rtl8169_pci_tbl[chip_idx].device_id)
			{
				...
				{
					sprintf( node_path, "net/eth/r8169-%d", cards_found );
					dev->node_handle = create_device_node( nDeviceID, tp->pci_dev->nHandle,
						node_path, &g_sDevOps, dev );
					kerndbg( KERN_DEBUG, "r8169_probe() Create node: %s\n", node_path );
