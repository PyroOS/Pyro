...
struct device
{
  ...
  unsigned char mtu;            /* Interface MTU value */
...
http://cesdis.gsfc.nasa.gov/atheos/misc/NWay.html
...
struct tulip_private {
  ...
  int saved_if_port;
  unsigned char pci_bus, pci_devfn, pci_func;
  int ttimer;
  int susp_rx;
  unsigned long nir;
...
static int tulip_start_xmit(PacketBuf_s *skb, struct device *dev);
static int tulip_refill_rx(struct device *dev);
static int tulip_rx(struct device *dev);
static int tulip_interrupt(int irq, void *dev_instance, SysCallRegs_s *regs);
...
void PCI_PrintInfo( PCI_Info_s* psInfo )
{
  printk( "----------------------------\n" );
...
int tulip_probe(int device_handle)
{
  int cards_found = 0;
  //	int pci_index = 0;
  ...
  for ( i = 0 ; psBus->get_pci_info( &sInfo, i ) == 0 ; ++i ) {
    ...
    /*
      vendor = read_pci_config(sInfo.nBus, sInfo.nDevice, sInfo.nFunction, PCI_VENDOR_ID);
      device = read_pci_config(sInfo.nBus, sInfo.nDevice, sInfo.nFunction, PCI_DEVICE_ID);
      subvendor = read_pci_config(sInfo.nBus, sInfo.nDevice, sInfo.nFunction, PCI_SUBSYSTEM_VENDOR_ID);
    ...
    */
    ...
    for (chip_idx = 0; pci_tbl[chip_idx].vendor_id; chip_idx++)
      if (sInfo.nVendorID == pci_tbl[chip_idx].vendor_id
          && (sInfo.nDeviceID & pci_tbl[chip_idx].device_id_mask) ==
          pci_tbl[chip_idx].device_id)
        break;
    if (pci_tbl[chip_idx].vendor_id == 0)
      continue;
	   
    printk("tulip_probe() found NIC\n");
    if (tulip_debug > 5) PCI_PrintInfo(&sInfo);
                                   ...
                                   int chip_idx, int board_idx)
{
  ...
  */
  ...
  if (chip_idx == DC21040) {
    ...
    /* DEC now has a specifiction (see Notes) but early board makers
  ...
  printk( "tulip Mac address : %02x%02x%02x%02x%02x%02x\n",
  ...
  //tp = (void *)(((long)kmalloc(sizeof(*tp), MEMF_KERNEL | MEMF_CLEAR /*GFP_KERNEL | GFP_DMA) + 7) & ~7*/)));
  tp = kmalloc(sizeof(*tp), MEMF_KERNEL /*GFP_KERNEL | GFP_DMA) + 7) & ~7*/);
  memset(tp, 0, sizeof(*tp));
  dev->priv = tp;
  
  spinlock_init( &tp->tx_spinlock, "tulip_tx_spinlock" );
...
//#define TULIP_NO_MEDIA_SWITCH
  ...
  /*  ???? 
  if (tulip_tbl[chip_idx].flags & HAS_PWRDWN)
    write_pci_config(pci_bus, pci_devfn, pci_func, 0x40, 4, 0x40000000);
  */
...
static void parse_eeprom(struct device *dev)
{
 ...
 subsequent_board:
  ...
  if (ee_data[27] == 0) {		/* No valid media table. */
    printk(KERN_INFO "%s: no valid media table!\n", dev->name);
    ...
    struct mediatable *mtable;
    u16 media = get_u16(p);
    printk(KERN_INFO "%s: parse_eeprom() media is : %d\n", dev->name, media);
    ...
    mtable = (struct mediatable *)
      kmalloc(sizeof(struct mediatable) + count*sizeof(struct medialeaf),
              MEMF_KERNEL /* GFP_KERNEL */);
    if (mtable == NULL) {
      printk(KERN_INFO "%s: horrible, impossible failure!\n", dev->name);
...
void tulip_wakup_tx( struct device* dev )
{
    struct tulip_private *tp = (struct tulip_private *)dev->priv;
    //printk("w\n");
    wakeup_sem( tp->tx_sema, true );
}

int tulip_wait_for_tx( struct device* dev )
{
    struct tulip_private *tp = (struct tulip_private *)dev->priv;
    //printk("s\n");
    return( sleep_on_sem( tp->tx_sema, 1000LL ) );
}
...
tulip_open(struct device *dev)
{
  ...
  printk(KERN_DEBUG "tulip_open() reset.\n");
  outl(0x00000001, ioaddr + CSR0);

  tp->tx_sema = create_semaphore("tulip_tx_sem", 0, 0); 
  dev->irq_handle = request_irq(dev->irq, &tulip_interrupt, NULL, SA_SHIRQ, dev->name, dev);
  
  if (dev->irq_handle < 0) {
    printk(KERN_DEBUG "request_irq() %d fail!, we won't continue.\n", dev->irq);
    return -EAGAIN;
  }
  ...
  if (tulip_debug > 1)
    printk(KERN_DEBUG "%s: tulip_open() irq %d with io_addr %8.8x.\n", dev->name, dev->irq, ioaddr);
  ...
  if (tulip_debug > 1)
    printk(KERN_DEBUG "%s: setup tx/rx descriptor %8.8x at %8.8x, %8.8x at %8.8x\n",
         dev->name, virt_to_bus(tp->tx_ring), (ioaddr + CSR4), virt_to_bus(tp->rx_ring), (ioaddr + CSR3));
  if (tulip_debug > 1)
    printk(KERN_DEBUG "%s: select a default media, dev->if_port is %d\n", dev->name, dev->if_port);
 ...
 media_picked:
  ...
  if (dev->if_port == 0  &&  tp->chip_id == DC21142) {
    ...
    printk(KERN_DEBUG "%s start_nway()\n", dev->name);	  
    t21142_start_nway(dev);
...
static void select_media(struct device *dev, int startup)
{
  ...
  if (mtable) {
    ...
    printk(KERN_INFO "%s: mleaf->type is %d\n", dev->name, mleaf->type);
    switch (mleaf->type) {
...
static void t21142_timer(unsigned long data)
{
  ...
  if (tulip_debug > 2)
    printk(KERN_INFO"%s: 21143 negotiation status %8.8x, dev->if_port is %d, media %s.\n",
           dev->name, csr12, dev->if_port,  medianame[dev->if_port]);
  if (media_cap[dev->if_port] & MediaIsMII) {
    check_duplex(dev);
    next_tick = 60*HZ;
  } else if (tp->nwayset) {
...
static void t21142_lnk_change(struct device *dev, int csr5)
{
  ...
  printk(KERN_INFO "%s: 21143 link status interrupt(csr12) %8.8x, CSR5 %8.8x, csr14 %8.8x.\n", 
...
#define __TULIP_DEBUG__  
#ifdef __TULIP_DEBUG__    // defined(way_too_many_messages)
  if (tulip_debug > 3) {
    int i;
    ...
    printk(KERN_DEBUG "  Tx ring %8.8x \n", (int)tp->tx_ring);
  }
#endif
...
static void tulip_init_ring(struct device *dev)
{
  ...
  for (i = 0; i < RX_RING_SIZE; i++) {
    ...
    PacketBuf_s *skb = alloc_pkt_buffer(PKT_BUF_SZ);
    tp->rx_skbuff[i] = skb;
    if (skb == NULL)
      break;
    skb->pb_nSize = 0;
    ...
    tp->rx_ring[i].buffer1 = virt_to_bus(skb->pb_pData);
  }
...
static int
tulip_start_xmit(PacketBuf_s *skb, struct device *dev)
{
  struct tulip_private *tp = (struct tulip_private *)dev->priv;
  int entry, flags;
  u32 flag;
...
again:
  ...
  if (test_and_set_bit(0, (void*)&dev->tbusy) != 0) {
    if ( spinunlock_and_suspend( tp->tx_sema, &tp->tx_spinlock, flags, 4000000LL ) < 0 ) {
      printk( "Error: tulip_wait_for_tx() timed out\n" );
      return 1;
    }
  ...
  tp->tx_ring[entry].buffer1 = virt_to_bus(skb->pb_pData);
  ...
  tp->tx_ring[entry].length = skb->pb_nSize | flag;
...
static int tulip_refill_rx(struct device *dev)
{
  ...
  for (; tp->cur_rx - tp->dirty_rx > 0; tp->dirty_rx++) {
    ...
    if (tp->rx_skbuff[entry] == NULL) {
      PacketBuf_s *skb;
      skb = tp->rx_skbuff[entry] = alloc_pkt_buffer(PKT_BUF_SZ);
      if (skb == NULL)
        break;
#ifndef __ATHEOS__			
      skb->dev = dev;			/* Mark as being used by this device. */
#endif			
      skb->pb_nSize = 0;
      tp->rx_ring[entry].buffer1 = virt_to_bus(skb->pb_pData);
      refilled++;
    }
...
#endif				
#ifndef __ATHEOS__
        eth_copy_and_sum(skb, tp->rx_skbuff[entry]->/*tail*/pb_pData, pkt_len, 0);
        skb_put(skb, pkt_len);
#else
        memcpy(skb_put(skb, pkt_len), (char *)bus_to_virt(tp->rx_ring[entry].buffer1),
               pkt_len);
#endif
...
#ifndef final_version
        if ((char *)bus_to_virt(tp->rx_ring[entry].buffer1) != temp)
          printk(KERN_ERR "%s: Internal fault: The skbuff addresses "
                 "do not match in tulip_rx: %4.4x vs. %4.4x / %4.4x.\n",
                 dev->name,
                 bus_to_virt(tp->rx_ring[entry].buffer1),
                 skb->pb_pHead, temp);
#endif
      }
      // skb->pb_nProtocol = 0; /* eth_type_trans(skb, dev); */
      // netif_rx(skb);
      if ( dev->packet_queue != NULL ) {
	skb->pb_uMacHdr.pRaw = skb->pb_pData;
	enqueue_packet( dev->packet_queue, skb );
      } else {
	printk(KERN_ERR "%s: tulip_rx() received packet to downed device!\n",
	       dev->name);
	if (skb) free_pkt_buffer( skb );
      }
...
static int tulip_close(struct device *dev)
{
  ...
  for (i = 0; i < RX_RING_SIZE; i++) {
    PacketBuf_s *skb = tp->rx_skbuff[i];
    tp->rx_skbuff[i] = 0;
...
static struct enet_statistics *tulip_get_stats(struct device *dev)
{
  ...
  return (struct enet_statistics *)&tp->stats;
}
...
static dev_node_t *tulip_attach(dev_locator_t *loc)
{
  ...
  if (dev) {
    dev_node_t *node = kmalloc(sizeof(dev_node_t), MEMF_KERNEL /*GFP_KERNEL*/);
...
static status_t tulip_open_dev( void* pNode, uint32 nFlags, void **pCookie )
{
  return( 0 );
}

static status_t tulip_close_dev( void* pNode, void* pCookie )
{
  return( 0 );
}

static status_t tulip_ioctl( void* pNode, void* pCookie, uint32 nCommand, void* pArgs, bool bFromKernel )
{
  ...
  switch( nCommand )
    ...
    case SIOCGIFHWADDR:
      ...
      break;
    default:
      printk( "Error: tulip_ioctl() unknown command %d\n", nCommand );
      nError = -ENOSYS;
      break;
    }
...
static int tulip_read( void* pNode, void* pCookie, off_t nPosition, void* pBuffer, size_t nSize )
{
  return( -ENOSYS );
}

static int tulip_write( void* pNode, void* pCookie, off_t nPosition, const void* pBuffer, size_t nSize )
{
  struct device* dev = pNode;
  PacketBuf_s* psBuffer = alloc_pkt_buffer( nSize );
  if ( psBuffer != NULL ) {
    memcpy( psBuffer->pb_pData, pBuffer, nSize );
    tulip_start_xmit( psBuffer, dev );
  }
  return( nSize );
}
...
{
  printk(KERN_DEBUG "tulip init with device id : %d\n", nDeviceID);
  return( tulip_probe( nDeviceID ) );
}
