...
SpinLock_s acpi_device_lock = INIT_SPIN_LOCK( "acpi_device_lock" );
...
static sem_id acpi_bus_drivers_lock;
	...
	struct acpi_driver	*driver)
{
	...
	kerndbg( KERN_DEBUG, "ACPI: Register driver %s\n", driver->name );
			       ...
			       acpi_handle handle, int type)
{
	...
	if (cid_list) {
		device->pnp.cid_list = kmalloc(cid_list->size, MEMF_KERNEL);
	...
	int			type)
{
	...
	device = kmalloc(sizeof(struct acpi_device), MEMF_KERNEL);
	if (!device) {
...
int acpi_scan_init(void)
{
	...
	acpi_bus_drivers_lock = create_semaphore( "acpi_bus_drivers_lock", 1, 0 );
