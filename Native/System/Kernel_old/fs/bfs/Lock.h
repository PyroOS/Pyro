...
class RecursiveLock {
	public:
		...
		{
			thread_id thread = sys_get_thread_id(NULL);
			if (thread == fOwner) {
				fOwnerCount++;
				return B_OK;
			}
...
#endif
				status = lock_semaphore(fSemaphore, SEM_NOSIG, timeout);
		...
		status_t Lock()
		{
			return LockWithTimeout(INFINITE_TIMEOUT);
		}
...
//			set_sem_owner(fSemaphore, B_SYSTEM_TEAM);
		...
		status_t LockWrite()
		{
			...
			if (readers < MAX_READERS) {
				...
				// the one for us
				status = lock_semaphore_ex(fSemaphore, readers <= 0 ? 1 : MAX_READERS - readers, SEM_NOSIG, INFINITE_TIMEOUT);
			}
			fWriteLock.Unlock();
...
//			set_sem_owner(fSemaphore, B_SYSTEM_TEAM);
		...
		status_t LockWrite()
		{
			return lock_semaphore_ex(fSemaphore, MAX_READERS, 0, 0);
		}
...
class SimpleLock {
	public:
		...
		{
			int32 thisThread = sys_get_thread_id(NULL);
			int32 current;
			while (1) {
				/*if (fHolder == -1) {
					current = fHolder;
		...
		bool IsLocked() const
		{
			return atomic_read( &fHolder ) == sys_get_thread_id( NULL);
		}
