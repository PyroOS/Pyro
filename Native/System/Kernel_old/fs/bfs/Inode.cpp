...
InodeAllocator::New(block_run *parentRun, mode_t mode, block_run &run, Inode **_inode)
{
	...
	node->create_time = HOST_ENDIAN_TO_BFS_INT64((bigtime_t)( get_real_time() / 1000000 ) << INODE_TIME_SHIFT);
...
Inode::WriteAt(Transaction *transaction, off_t pos, const uint8 *buffer, size_t *_length)
{
	...
	Node()->last_modified_time = (bigtime_t)( get_real_time() / 1000000 ) << INODE_TIME_SHIFT;
