					  ...
					  unsigned int flags )
{
	...
	session->buffer = (char*)malloc( POP3_BUFFER_SIZE + 1 );
			...
			void **data )
{
	/* Get the message data from the server.  Dump the data into a buffer
	   and point *data at it. */
     	...
     	int ret, size = 0;		/* Size of the message in bytes */
	...
	char id[256];			/* ASCIIzed Message ID */
	char *message;			/* Message data from RETR */

	if( NULL == session || NULL == *data )
		return EINVAL;

	/* Get the message size */
	ret = pop3_get_message_size( session, message_id, &size );
	if( ret )
		return ret;
	...
	{
		__pop3_start_timer( session );
		bytes = read( session->sock_fd, message + bytes_total, size - bytes_total );
		bytes_total += bytes;
