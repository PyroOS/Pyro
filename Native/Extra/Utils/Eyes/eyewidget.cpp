...
EyeWidget::EyeWidget( Rect cFrame, const String& cName, uint32 nResizeMask, uint32 nFlags ) : View(cFrame, cName, nResizeMask, nFlags)
{
	// Create pop up menu
	...
	pcContextMenu->AddItem("Quit", new Message(M_MENU_QUIT));
...
void EyeWidget::Paint( const Rect& cUpdateRect )
{
	...
	DrawFillEllipse( width*3, height, width-5, height-5 );

	// Draw outer eye	
	SetFgColor( 0, 0, 0 );
	DrawEllipse( width, height, width-5, height-5 );
	DrawEllipse( width*3, height, width-5, height-5 );
