...
IconList::IconList( Rect cFrame, int nFlags, Handler *pcHandler ) : View( cFrame, "icon_list", nFlags )
{
	...
	m_pcVScrollBar = new ScrollBar( Rect( cFrame.Width() - ( IL_SCROLL_WIDTH + 2 ), 2, cFrame.Width() - 2, cFrame.Height() - 2 ), "icon_list_vscroll", new Message( ID_SETTINGS_SCROLL_V ), 0, FLT_MAX, VERTICAL, CF_FOLLOW_RIGHT | CF_FOLLOW_TOP | CF_FOLLOW_BOTTOM );
...
status_t IconList::InnerView::Select( int nIndex, bool bNotify )
{
	...
	if( bNotify )
	{
		...
		cMessenger.SendMessage( ID_SETTINGS_SELECT, this );
	}
	...
	switch( pcMessage->GetCode() )
	{
		case ID_SETTINGS_SELECT:
		{
			...
			if( pcMessage->FindInt32( "selection", &nSelected ) == EOK )
			{
				...
				m_pcSelected->Update( pcSettings, pcWebSettings );

				m_pcFrameView->AddChild( m_pcSelected );

				delete( pcSettings );
				delete( pcWebSettings );
			}
			break;
		}
